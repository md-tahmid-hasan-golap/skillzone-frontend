...
import { BookOpen, Users, Star, Edit, Trash2, ArrowRight, Loader2, Shield } from "lucide-react";
...
const ManageCoursesList = () => {
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries(["allSkills"]);
    },
    onError: (error) => {
  ...
  const handleDelete = (id) => {
    if (confirm("Are you sure you want to delete this course as an Admin?")) {
  ...
  if (allCourses.length === 0) {
    return (
      <div className="w-full text-center py-20">
        ...
        <p className="text-muted-foreground mt-2">There are no courses on the platform.</p>
      </div>
    );
  }
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
      {allCourses.map((skill) => {
        const { _id, title, instructor, price, duration, rating, image, description, lessons, enrolledStudents, status } = skill;
        ...
        return (
          ...
          >
            <div className="absolute top-0 left-0 w-full h-1 bg-gradient-to-r from-red-500 to-orange-500 opacity-0 group-hover:opacity-100 transition-opacity duration-300 z-20" />
            ...
            <div className="relative aspect-video w-full overflow-hidden bg-muted">
              ...
              />
              <div className="absolute top-3 left-3 bg-red-500/90 backdrop-blur-md px-2.5 py-1 rounded-md border border-red-500/20 text-xs font-semibold text-white shadow-sm z-10 flex items-center gap-1">
